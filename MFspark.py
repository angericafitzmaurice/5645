...
def joinAndPredictAll(R,U,V,N):
    ...
    return R2.join(V, numPartitions=N) \
            .map(lambda pair:(pair[1][0][0],
                              pair[0],
                              pred_diff(pair[1][0][1],
                                        pair[1][0][2],
                                        pair[1][1]),
                              pair[1][0][2],
                              pair[1][1]))
...
def normSqRDD(profileRDD,param):
    """ Receives as input an RDD of profiles (e.g., U) as well as a parameter (e.g., λ) and computes the square of norms:
        ...
        The return value is:
            λ Σ_i ||ui||_2^2
    """
    return param * profileRDD.map(lambda pair: np.dot(pair[1], pair[1])).reduce(add)
...
def adaptU(joinedRDD,gamma,lam,N):
    ...
    return joinedRDD.map(lambda x: (x[0], (gradient_u(x[2], x[3], x[4]), x[3]))) \
                    ...
                    .map(lambda pair: (pair[0],
                                       pair[1][1] - gamma * pair[1][0] - gamma * 2 * lam * pair[1][1]))
...
def adaptV(joinedRDD,gamma,mu,N):
    ...
    return joinedRDD.map(lambda x: (x[1], (gradient_v(x[2], x[3], x[4]), x[4]))) \
                    .reduceByKey(lambda x, y: (x[0] + y[0], (x[1] + y[1]) / 2), numPartitions=N) \
                    .map(lambda pair: (pair[0],
                                       pair[1][1] - gamma * pair[1][0] - gamma * 2 * mu * pair[1][1]))
